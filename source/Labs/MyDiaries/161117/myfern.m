...
p  = [ .85  .92  .99  1.00]; %probabilità comulative
...
while ~get(stop,'value')
   ...
   for j = 1:N
       r = rand;
       if r < p(1)            %shooting method: estraggo un numero e a seconda di dove cade questo numero faccio cose diverse
          x = A1*x + b1;
       elseif r < p(2)
          x = A2*x + b2;
       elseif r < p(3)
       ...
       X(:,j) = x; %riempio una colonna alla volta
   end
   plot(X(1),X(2),'.','markersize',4,'color',darkgreen)
   drawnow
   cnt = cnt + N;
